...
const allowedOrigins = ['http://localhost:5173', 'https://binaramics.com:4343'];
...
function handleDisconnect() {
  ...
  const query = 'SELECT * FROM environ LIMIT 1';
  ...
  app.get('/getVaultDepositByOwner', async (req, res) => {
    try {
      const wallet = req.query.wallet; 
  
      const query = `
        SELECT SQUADS_DEPOSIT_WALLET
        FROM environ
        WHERE WALLET = ?
      `;
  
      const [rows] = await connection.promise().query(query, [wallet]);
  ...
  app.post('/newVault', async (req, res) => {
    try {
      const { vaultDepositWallet, vaultPDA, vaultCreator, vaultOwner } = req.body;
  
      if (!vaultDepositWallet) {
        return res.status(400).json({ error: 'Missing vaultDepositWallet in the request body' });
      }

      if (!vaultPDA) {
        return res.status(400).json({ error: 'Missing vaultPDA in the request body' });
      }

      if (!vaultCreator) {
        return res.status(400).json({ error: 'Missing vaultCreator in the request body' });
      }

      if (!vaultOwner) {
        return res.status(400).json({ error: 'Missing vaultOwner in the request body' });
      }
  
      let swaps = [];

      // Check if the wallet exists in the database
      const [rows] = await connection.promise().query('SELECT WALLET FROM environ WHERE WALLET = ?', [vaultOwner]);
  
      if (rows.length > 0) {
        return res.status(400).json({ error: 'Vault already linked' });
      } else {
        await connection.promise().query('INSERT INTO environ (WALLET, SQUADS_DEPOSIT_WALLET, VAULT_PDA, VAULT_CREATOR, SWAPS, GOALS) VALUES (?, ?, ?, ?, ?, ?)', [vaultOwner, vaultDepositWallet, vaultPDA, vaultCreator, JSON.stringify(swaps), 0]);
      }
...
server.listen(port, () => {
  console.log(`The Environ API is now running on port ${port}`);
});